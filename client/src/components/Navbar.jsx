...
function Navbar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      if (res.data.success) {
        // Adjusting this part to access res.data
        dispatch(removeUser());
  ...
  return (
    <div className="fixed flex w-full h-[10vh] backdrop-blur-2xl z-[10]">
      ...
      <div className="w-[30%] h-full  justify-between py-7 text-white hidden md:flex ">
        <NavLink
          to="/profile"
          className="text-xl font-normal text-white hover:scale-110 cursor-pointer  duration-[0.3s]"
        >
          Profile
        </NavLink>
        <NavLink
          to="/connections"
          className="text-xl font-normal  hover:scale-110  cursor-pointer duration-[0.3s] "
        >
          My Connections
        </NavLink>
        <h1
          onClick={handleLogout}
          className="text-xl font-normal  cursor-pointer  hover:scale-110 duration-[0.3s]"
        >
          Logout
        </h1>
        <NavLink
      ...
      <div className="w-[30%] h-full px-20 py-6 flex gap-4">
        ...
        <img
          src={
            user
              ? user.userimage
              : "https://img.freepik.com/free-vector/follow-me-social-business-theme-design_24877-52233.jpg?semt=ais_hybrid"
          }
          alt="user_image"
          className="w-[3.2vw] h-[3.2vw] rounded-xl mt-[-0.6vw] "
        />
        <h1 className=" cursor-pointer text-white text-xl font-bold hidden md:flex">
          {" "}
          {!openPanel ? (
            <GiHamburgerMenu
              size={28}
      ...
      <h1 className="cursor-pointer text-white text-xl font-bold md:hidden">
        {" "}
        {!openPanel ? (
          <GiHamburgerMenu
            size={28}
      ...
      {openPanel && (
        ...
        <NavLink
            to="/"
            className="w-full px-5 py-2 rounded-lg bg-black text-white text-xl font-semibold "
